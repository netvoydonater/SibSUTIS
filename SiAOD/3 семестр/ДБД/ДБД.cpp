...
bool B2INSERT(Vertex*& p, int data) 
{
    if (p == NULL) 
    {
        p = new Vertex;
...
Vertex* create_DBD(int n, int * Arr)
{
	Vertex* Root = NULL;
	for (int i = 0; i < n; i++)
	{
...
int main()
{
    ...
    FillRand(array1, size);  
	Vertex* DBD_Tree = create_DBD(size, array1);
	int DBD_Size = CalculateTreeSize(DBD_Tree);
	int DBD_Sum = CalculateTreeSum(DBD_Tree);
    int DBD_Height = CalculateTreeHeight(DBD_Tree);
	int DBD_PathLength = CalculatePathLenght(DBD_Tree, 1); 
	double DBD_AvgHeight = static_cast<double>(DBD_PathLength) / DBD_Size;
    cout << "Обход ДБД слева направо:\n";
    TreeRight(DBD_Tree);
    cout << endl;
    ...
    cout << setw(10) << "ДБД" << setw(10) << DBD_Size << setw(15) << DBD_Sum << setw(10) << DBD_Height << setw(15) << DBD_AvgHeight << endl;
